...
+function ($) { "use strict";
    ...
    OctoberLeaves.prototype.start = function() {
        ...
        for (var i = 0 ;i < this.options.numberOfLeaves;i++){
            ...
            width[i] = Math.round(this.options.size + 20);
            height[i] = Math.round(this.options.size + 20);
            ...
            // Speed of leaves
            Speed[i] = Math.random() * this.options.speedC + 1;
    ...
    OctoberLeaves.prototype.stop = function() {
        if (this.timer) {
            ...
            $('.october-leaf').fadeOut(200, function(){
                $(this).remove()
            })
        }
    }
    ...
    OctoberLeaves.prototype.leafCycle = function() {
        for (var i = 0; i < this.options.numberOfLeaves; i++) {
            ...
            // Strafe
            var strafeY = Math.sin(90 * Math.PI / 500),
                strafeX = Math.cos(CStrafe[i]);
            ...
            if (Ypos[i] > this.winHeight - (width[i] + height[i]/2)){
                Ypos[i] =- 50;
                Xpos[i] = Math.round(Math.random() * this.winWidth-width[i] * 1);
